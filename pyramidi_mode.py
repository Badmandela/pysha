...
BUTTON_UPPER_ROW_2 = push2_python.constants.BUTTON_UPPER_ROW_2

# push2_python.Push2Buttons.set_button_color(push2_python.constants.BUTTON_UPPER_ROW_2, definitions.ORANGE)
# push2_python.Push2Buttons.set_button_color('Upper Row 2', 'white')
...
class MIDICCControl(object):
    ...
    def __init__(self, cc_number, name, section_name, get_color_func, send_midi_func):
        # push2_python.buttons.Push2Buttons.set_button_color('Upper Row 2', definitions.ORANGE)
        # app.buttons.set_button_color(BUTTON_UPPER_ROW_2, 'black')
    ...
    def draw(self, ctx, x, y):
        ...
        if self.name == '':
            ...
            if self.value > 10 and self.value < 117:
            ...
            show_title(ctx, x, y, self.name, color=definitions.get_color_rgb_float(definitions.TURQUOISE))
            show_value(ctx, x, y + 10, self.value, color=definitions.get_color_rgb_float(definitions.TURQUOISE))
        elif self.name == 'TAPE':
            ...
            show_value(ctx, x, y+10, self.value, color=color)
...
# Inner circle
        ...
        if self.name == 'INSTRUMENT':
            ...
            ctx.set_source_rgb(*definitions.get_color_rgb_float(definitions.TURQUOISE_DARK))
        elif self.name == 'TAPE':
...
# Outer circle
        ...
        if self.name == 'LPF':
            ...
            ctx.set_source_rgb(*definitions.get_color_rgb_float(definitions.TURQUOISE))
        elif self.name == 'TAPE':
...
class PyramidiMode(PyshaMode):
    ...
    def initialize(self, settings=None):
        ...
        for i in range(0, 64):
            ...
            if i % 8 == 0:
                # data['instrument_name'] = 'MASTER'
                data['instrument_short_name'] = 'MASTER'
                data['color'] = definitions.ROOT_KEY
                data['default_layout'] = LAYOUT_MELODIC
